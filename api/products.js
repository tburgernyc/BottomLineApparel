...
export default async function handler(req, res) {
  ...
  // Step 2: Fetch full detail (with sync_variants) for each product in parallel
  // We need variant detail for retail_price and preview image.
  let detailedProducts;
  try {
    const detailFetches = productList.map(p =>
    ...
    detailedProducts = (await Promise.all(detailFetches)).filter(Boolean);
  } catch (err) {
