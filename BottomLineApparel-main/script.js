...
document.addEventListener('DOMContentLoaded', () => {
  ...
  /* ════════════════════════════════════════════════
     HERO POPUP CARD — Phase 6
     Reveals 8 seconds after the hero video begins playing.
     Timer resets from the play event so the delay is always
     measured from the moment the visitor actually sees video.
     ════════════════════════════════════════════════ */
  const heroPopupCard    = document.getElementById('hero-popup-card');
  const heroPopupDismiss = document.getElementById('hero-popup-dismiss');
  const heroVideo        = document.getElementById('hero-video') ||
                           document.querySelector('.hero-video-bg');
  ...
  if (heroPopupCard) {
    ...
    if (heroVideo) {
      heroVideo.addEventListener('play', () => {
        ...
        popupTimer = setTimeout(revealPopup, 8000);
      }, { once: true });
    }

    // Close / dismiss
    if (heroPopupDismiss) {
      heroPopupDismiss.addEventListener('click', () => {
        heroPopupCard.classList.remove('active');
        heroPopupCard.style.pointerEvents = 'none';
      });
    }

    // 3D tilt — card responds to cursor position within its own bounds
    heroPopupCard.addEventListener('mousemove', e => {
      const rect = heroPopupCard.getBoundingClientRect();
      const cx   = rect.left + rect.width  / 2;
      const cy   = rect.top  + rect.height / 2;
      const rx   = ((e.clientY - cy) / (rect.height / 2)) * -6;
      const ry   = ((e.clientX - cx) / (rect.width  / 2)) *  9;
      heroPopupCard.style.transform  = `perspective(900px) rotateX(${rx}deg) rotateY(${ry}deg) translateZ(6px)`;
      heroPopupCard.style.transition = 'transform 0.09s linear';
    });
    heroPopupCard.addEventListener('mouseleave', () => {
      heroPopupCard.style.transform  = '';
      heroPopupCard.style.transition = 'transform 0.65s cubic-bezier(0.23,1,0.32,1)';
    });
  }

  /* ════════════════════════════════════════════════
     HERO PRODUCT VISUAL PARALLAX — Phase 2 → 6
     Now targets the visual inside the popup card.
     Fine-grained tilt driven by cursor over the whole hero.
     ════════════════════════════════════════════════ */
  const heroVisualEl = document.getElementById('hero-visual');
  const heroSection  = document.getElementById('hero');
  if (heroVisualEl && heroSection) {
    heroSection.addEventListener('mousemove', e => {
      if (!heroPopupCard || !heroPopupCard.classList.contains('active')) return;
